...
isset($_GET['pic'])?$pic = $_GET['pic']:$pic = NULL;
switch($mode) {
...
isset($_GET['s'])?$get = $_GET['s']:$get = NULL;
echo($get);
$get=base64_decode(str_replace(" ","+",$get ?? ''),true);
echo($get);
$wangzhi=$_SERVER['HTTP_REFERER'];//这里获取当前网址
//here is ip 
$url="https://api.asilu.com/ip/?ip=".$ip;
...
imagettftext($im, 16, 0, 10, 40, $red, $font,'欢迎您来自'.$dz.'的朋友');
imagettftext($im, 16, 0, 10, 72, $red, $font, '今天是'.date('Y年n月j日')."  星期".$weekarray[date("w")]);//当前时间添加到图片
imagettftext($im, 16, 0, 10, 104, $red, $font,'您的IP是:'.$ip."(".$wl.")");//ip
